...
// El banco no es un personaje ni acumula saldo: sirve de destino para pagos de los jugadores y de emisor cuando el DJ ingresa o retira créditos.
...
function personajeLabel(id) {
  ...
  if (id === BANCO_ID) return BANCO_NOMBRE;
...
function statsHtml(stats, cls) {
  return (stats || [])
    .map((s) => `<div class="${cls}">${esc(s.label)}: ${esc(s.value)}</div>`)
    .join("");
}
  ...
  return (
    {
      ...
      pocket: "equip-skin",
      ship: "ship-skin",
      engineering: "engineering-skin",
      cargo: "cargo-skin",
      list: "list-skin",
      comms: "comms-skin",
      circuit: "circuit-skin",
      pharma: "pharma-skin",
      medical: "medical-skin",
      electric: "electric-skin",
      communist: "communist-skin",
      showcase: "showcase-skin",
    }[skin] || "ammo-skin"
  );
}
  ...
  return (
    {
      ...
      pocket: "equip-inner",
      ship: "ship-inner",
      engineering: "eng-inner",
      cargo: "cargo-inner",
      list: "list-inner",
      comms: "comms-inner",
      circuit: "circuit-inner",
      pharma: "pharma-inner",
      medical: "medical-inner",
      electric: "electric-inner",
      communist: "communist-inner",
...
function catalogKey(catId, name) {
  return catId + "|" + name;
}
  ...
  fetch("../tienda/data.json")
    ...
    .then((data) => {
      (data.categorias || []).forEach((cat) => {
        SUBCATS_BY_CAT[cat.id] = (cat.subcategorias || []).map((sub) => ({
          id: sub.id,
          label: sub.label,
        }));
    ...
    .catch((err) =>
      console.error("No se pudo cargar el catálogo para el inventario:", err),
    );
}
...
function buildInventoryCard(skin, it, qty, peticionHtml) {
  ...
  const descHtml = it.effect
    ? `<div class="inv-effect">${esc(it.effect)}</div>`
    : "";
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      try {
        ...
        console.error("Error resolviendo personaje:", e);
      }
    }
    updateSessionUI();
  });
...
function initDataListeners() {
  ...
  onValue(ref(db, "tienda/shipCredits"), (snap) => {
    ...
    document.getElementById("shipBalance").textContent =
      formatCr(shipCredits) + " cr";
  ...
  onValue(ref(db, "tienda/objetosLog"), (snap) => {
    const val = snap.val() || {};
    objetosLogData = Object.keys(val)
      .map((key) => ({ key, ...val[key] }))
      .sort((a, b) => (b.ts || 0) - (a.ts || 0));
    renderObjetosLog();
  });
...
function renderObjetosLog() {
  ...
  const searchTerm = (document.getElementById("objetosSearch").value || "")
    .trim()
    .toLowerCase();
  ...
  countEl.textContent =
    ...
    (filtered.length !== objetosLogData.length
      ? ` (de ${objetosLogData.length})`
      : "");

  if (!filtered.length) {
    list.innerHTML = "";
    empty.style.display = "block";
    empty.textContent = objetosLogData.length
      ? "Sin resultados para ese filtro."
      : "Todavía no se ha registrado ningún movimiento de objetos.";
    return;
  }
  ...
  list.innerHTML = filtered
    .map((l) => {
      ...
      if (l.tipo === "transferencia") {
        ...
        desc = `movió de ${deLabel} a ${aLabel}`;
        cls = "transferencia";
      } else if (l.tipo === "creacion") {
        const propietarioTxt = l.propietario
          ? ` → ${l.propietario === "nave" ? "la nave" : personajeLabel(l.propietario)}`
          : "";
        desc = `creó${propietarioTxt}`;
        cls = "deposito";
      } else {
        // consumo
        desc = `consumió (de ${l.propietario === "nave" ? "la nave" : personajeLabel(l.propietario)})`;
        cls = "retirada";
      }
      return `
...
function agruparCompras(raw) {
  ...
  raw.forEach((c) => {
    if (c.lineas) {
      const lineas = Array.isArray(c.lineas)
        ? c.lineas
        : Object.values(c.lineas);
      const total =
        typeof c.total === "number"
          ? c.total
          : lineas.reduce((s, l) => s + (l.total || 0), 0);
    ...
    for (let i = out.length - 1; i >= 0 && i >= out.length - 15; i--) {
      ...
      if (
        g.legacy &&
        g._sig === sig &&
        Math.abs((g._minTs || 0) - (c.ts || 0)) <= 10000
      ) {
        grupo = g;
        break;
      }
...
function renderCompras() {
  ...
  const searchTerm = (document.getElementById("comprasSearch").value || "")
    .trim()
    .toLowerCase();
  ...
  const filtered = todas.filter((c) => {
    ...
    if (searchTerm) {
      ...
      const haystack =
        `${nombres} ${personajeLabel(c.comprador)}`.toLowerCase();
      if (!haystack.includes(searchTerm)) return false;
    }
    return true;
  });
  ...
  if (!filtered.length) {
    list.innerHTML = "";
    empty.style.display = "block";
    empty.textContent = todas.length
      ? "Sin resultados para ese filtro."
      : "Todavía no se ha registrado ninguna compra.";
    return;
  }
  empty.style.display = "none";
  ...
  const toggle = (ev) => {
    ...
    if (nowOpen) comprasAbiertas.add(key);
    else comprasAbiertas.delete(key);
  };
  list.onclick = toggle;
...
function renderMoveForm() {
  ...
  document.getElementById("movePersonajeLabel").textContent =
    personajeLabel(currentPersonaje);
  const personal =
    (personajesData[currentPersonaje] &&
      personajesData[currentPersonaje].credits) ||
    0;
  document.getElementById("movePersonalCredits").textContent =
    formatCr(personal);

  const targetSelect = document.getElementById("moveTargetSelect");
  const others = PERSONAJES.filter(
    (p) => p.id !== currentPersonaje && p.id !== "nave",
  );
  const previo = targetSelect.value;
  targetSelect.innerHTML =
    others
      .map((p) => `<option value="${p.id}">${esc(p.label)}</option>`)
      .join("") +
    `<option value="${BANCO_ID}">🏦 ${esc(BANCO_NOMBRE)}</option>`;
  if (
    previo &&
    Array.from(targetSelect.options).some((o) => o.value === previo)
  )
    targetSelect.value = previo;
}
...
function renderLedger() {
  ...
  countEl.textContent =
    ledgerData.length + (ledgerData.length === 1 ? " REGISTRO" : " REGISTROS");
  ...
  list.innerHTML = ledgerData
    .map((l) => {
      const who =
        l.tipo === "ajusteDJ" ? BANCO_NOMBRE : personajeLabel(l.personaje);
      const dest = (txt) => `<span class="ledger-dest">${esc(txt)}</span>`;
      let accionTxt, sign, cls;
      if (l.tipo === "deposito") {
        accionTxt = "metió al bote";
        sign = "+";
        cls = "deposito";
      } else if (l.tipo === "retirada") {
        accionTxt = "sacó del bote";
        sign = "−";
        cls = "retirada";
      } else if (l.tipo === "ajusteDJ") {
        const destinoTxt =
          l.hacia === "nave" ? "la nave" : personajeLabel(l.hacia);
        accionTxt = `${l.signo > 0 ? "ingresó créditos a" : "cargó créditos a"} ${dest(destinoTxt)}`;
        sign = l.signo > 0 ? "+" : "−";
        cls = l.signo > 0 ? "deposito" : "retirada";
      } else {
        // transferencia
        accionTxt = `envió a ${dest(personajeLabel(l.hacia))}`;
        sign = "→";
        cls = "transferencia";
      }
      const conceptoHtml = l.concepto
        ? `<span class="ledger-concept-badge">${esc(l.concepto)}</span>`
        : "";
      const notaHtml = l.nota
        ? `<div class="ledger-nota">“${esc(l.nota)}”</div>`
        : "";
      return `
...
function moveCredits(tipo) {
  ...
  runTransaction(sourceRef, (current) => {
    ...
    .then(() => {
      showToast(
        tipo === "deposito"
          ? "Créditos depositados en el bote común."
          : "Créditos retirados del bote común.",
      );
      amountInput.value = "";
...
function transferCredits() {
  ...
  const esBanco = targetId === BANCO_ID;
  const targetRef = esBanco
    ? null
    : ref(db, `tienda/personajes/${targetId}/credits`);
  ...
  runTransaction(sourceRef, (current) => {
    ...
    .then((result) => {
      ...
      if (esBanco) return; // el banco no acumula saldo: el dinero sale de circulación
      return runTransaction(targetRef, (current) => {
        const currentVal = typeof current === "number" ? current : 0;
        return currentVal + amount;
      });
...
function attachInvActionListeners(ctx) {
  ...
  if (takeBtn)
    takeBtn.addEventListener("click", () =>
      runInvAction(takeBtn, errEl, () =>
        transferItem(ctx, getAmount(), "nave", currentPersonaje),
      ),
    );
  if (giveBtn)
    giveBtn.addEventListener("click", () =>
      runInvAction(giveBtn, errEl, () =>
        transferItem(ctx, getAmount(), currentPersonaje, "nave"),
      ),
    );
  if (consumeBtn)
    consumeBtn.addEventListener("click", () =>
      runInvAction(consumeBtn, errEl, () => consumeItem(ctx, getAmount())),
    );
  if (sendBtn)
    sendBtn.addEventListener("click", () => {
      const targetId = document.getElementById("invActTargetSelect").value;
      runInvAction(sendBtn, errEl, () =>
        transferItem(ctx, getAmount(), currentPersonaje, targetId),
      );
    });
}
...
async function transferItem(ctx, amount, fromId, toId) {
  ...
  const existingKey = Object.keys(data).find(
    (k) => data[k].catId === ctx.catId && data[k].name === ctx.name,
  );
  const fecha = nowFecha();

  if (existingKey) {
    await update(ref(db), {
      [`tienda/personajes/${toId}/inventario/${existingKey}/qty`]:
        (data[existingKey].qty || 0) + amount,
...
async function consumeItem(ctx, amount) {
  ...
  const qtyRef = ref(
    db,
    `tienda/personajes/${ctx.ownerId}/inventario/${ctx.invKey}/qty`,
  );
  ...
  if (result.snapshot.val() === 0) {
    await update(ref(db), {
      [`tienda/personajes/${ctx.ownerId}/inventario/${ctx.invKey}`]: null,
    });
  }
...
function renderAdminConceptChips() {
  ...
  if (!box) return;
  const chip = (t, cls) =>
    `<button type="button" class="concept-chip ${cls}" data-text="${esc(t)}">${esc(t.trim())}${t.endsWith(" ") ? "…" : ""}</button>`;
  box.innerHTML =
...
function renderAdminTab() {
  ...
  targetSelect.innerHTML =
    `<option value="${ADMIN_TODOS}">👥 Todos los jugadores (cada uno recibe la cantidad)</option>` +
    targetOptionsHtml();
  targetSelect.addEventListener("change", renderAdminCreditsInfo);
  renderAdminCreditsInfo();
  renderAdminConceptChips();

  document.getElementById("adminCreditsAdd").onclick = () =>
    adminAdjustCredits(1);
  document.getElementById("adminCreditsSub").onclick = () =>
    adminAdjustCredits(-1);
  ...
  notesTarget.innerHTML = PERSONAJES.map(
    (p) => `<option value="${p.id}">${esc(p.label)}</option>`,
  ).join("");
  notesTarget.addEventListener("change", loadAdminNote);
  document
    .getElementById("adminNotesSave")
    .addEventListener("click", saveAdminNote);
  loadAdminNote();
}
...
async function loadAdminNote() {
  ...
  try {
    ...
    document.getElementById("adminNotesText").value = snap.exists()
      ? snap.val()
      : "";
...
function targetSummary(targetId) {
  ...
  const items = data.inventario
    ? Object.entries(data.inventario).map(([k, v]) => ({ ...v, _key: k }))
    : [];
...
function invSummaryHtml(items, targetId) {
  if (!items.length)
    return `<div class="admin-inv-empty">Sin objetos en su inventario.</div>`;
  return (
    `<div class="admin-inv-list">` +
    items
      .slice()
...
async function adminAdjustItemQty(targetId, key, delta) {
  ...
  if (result.committed && result.snapshot.val() === 0) {
    await update(ref(db), {
      [`tienda/personajes/${targetId}/inventario/${key}`]: null,
    });
  }
}
...
async function adminRemoveInvItem(targetId, key, name) {
  if (
    !confirm(
      `¿Eliminar "${name}" del inventario de ${targetId === "nave" ? "la nave" : personajeLabel(targetId)}?`,
    )
  )
    return;
  ...
  showToast(
    `${name} eliminado del inventario de ${targetId === "nave" ? "la nave" : personajeLabel(targetId)}.`,
  );
}
...
function renderAdminCreditsInfo() {
  ...
  if (targetId === ADMIN_TODOS) {
    const filas = PERSONAJES.filter((p) => p.id !== "nave")
      .map((p) => {
...
async function adminAdjustCredits(sign) {
  ...
  const amount = Math.max(
    1,
    Math.floor(Number(document.getElementById("adminCreditsAmount").value)) ||
      0,
  );
  if (!amount) {
    errEl.textContent = "Introduce una cantidad válida.";
...
function buildAdminItemForm() {
  const catOptions = CATEGORIAS.map(
...
function renderAdminStatRows() {
  ...
  box.querySelectorAll(".craft-material-row").forEach((rowEl) => {
    ...
    rowEl
      .querySelector('[data-role="stat-label"]')
      .addEventListener("input", (ev) => {
        adminItemState.stats[idx].label = ev.target.value;
      });
    rowEl
      .querySelector('[data-role="stat-value"]')
      .addEventListener("input", (ev) => {
        adminItemState.stats[idx].value = ev.target.value;
      });
    rowEl
      .querySelector('[data-role="stat-remove"]')
      .addEventListener("click", () => {
        adminItemState.stats.splice(idx, 1);
        renderAdminStatRows();
      });
  });
}
...
function wireAdminItemForm() {
  ...
  const applyCat = (catId) => {
    ...
    adminItemState.stats = (CATEGORIA_STAT_PRESETS[catId] || []).map(
      (label) => ({ label, value: "" }),
    );
    renderAdminStatRows();
  };
  ...
  document
    .getElementById("admin-item-addStat")
    .addEventListener("click", () => {
  ...
  document
    .getElementById("admin-item-submit")
    .addEventListener("click", submitAdminItem);
}
...
async function submitAdminItem() {
  ...
  const subcatId = subcatField.classList.contains("hidden")
    ? null
    : document.getElementById("admin-item-subcat").value;
  const desc = document.getElementById("admin-item-desc").value.trim();
  const qty = Math.max(
    1,
    Math.floor(Number(document.getElementById("admin-item-qty").value)) || 1,
  );
  ...
  try {
    ...
    const data = snap.exists() ? snap.val() : {};
    const existingKey = Object.keys(data).find(
      (k) => data[k].catId === catId && data[k].name === name,
    );
    const fecha = nowFecha();

    if (existingKey) {
      await update(ref(db), {
        [`tienda/personajes/${targetId}/inventario/${existingKey}/qty`]:
          (data[existingKey].qty || 0) + qty,
        [`tienda/personajes/${targetId}/inventario/${existingKey}/fecha`]:
          fecha,
      });
    } else {
    ...
    showToast(
      `${name} entregado a ${targetId === "nave" ? "la nave" : personajeLabel(targetId)}.`,
    );
...
const artifactState = {
  stats: [
    { label: "Bonificador", value: "" },
    { label: "Coste de Tensión de Sistema", value: "" },
  ],
};
...
function renderArtifactStatRows() {
  ...
  box.innerHTML = artifactState.stats
    .map(
      (row, i) => `
    <div class="craft-material-row" data-idx="${i}">
  ...
  box.querySelectorAll(".craft-material-row").forEach((rowEl) => {
    ...
    rowEl
      .querySelector('[data-role="stat-label"]')
      .addEventListener("input", (ev) => {
        artifactState.stats[idx].label = ev.target.value;
      });
    rowEl
      .querySelector('[data-role="stat-value"]')
      .addEventListener("input", (ev) => {
        artifactState.stats[idx].value = ev.target.value;
      });
    rowEl
      .querySelector('[data-role="stat-remove"]')
      .addEventListener("click", () => {
        artifactState.stats.splice(idx, 1);
...
function wireArtifactForm() {
  ...
  document
    .getElementById("artifact-submit")
    .addEventListener("click", submitArtifact);
}
...
async function submitArtifact() {
  ...
  const procedencia = document
    .getElementById("artifact-procedencia")
    .value.trim();
  ...
  artifactState.stats
    .filter((s) => s.label.trim() && s.value.trim())
    .forEach((s) =>
      stats.push({ label: s.label.trim(), value: s.value.trim() }),
    );
  ...
  try {
    ...
    const data = snap.exists() ? snap.val() : {};
    const existingKey = Object.keys(data).find(
      (k) => data[k].catId === "artefacto" && data[k].name === name,
    );
    const fecha = nowFecha();
    ...
    if (existingKey) {
      ...
      await update(ref(db), {
        [`tienda/personajes/${targetId}/inventario/${newKey}`]: {
          ...
          qty: 1,
          catId: "artefacto",
          origen: "Artefacto pretecnológico",
          fecha,
          custom: true,
          effect: desc || null,
          stats,
    ...
    await update(ref(db), {
      [`tienda/objetosLog/${logKey}`]: {
        ...
        qty: 1,
        personaje: "admin",
        facility: "Artefacto pretecnológico",
        propietario: targetId,
        fecha,
        ts: Date.now(),
      },
    ...
    showToast(
      `${name} otorgado a ${targetId === "nave" ? "la nave" : personajeLabel(targetId)}.`,
    );
...
const FACILITIES = {
  taller: {
    label: "Taller",
    allowed: ["kael", "wulfram", "admin"],
    bodyId: "tallerBody",
  },
  laboratorio: {
    label: "Laboratorio",
    allowed: ["nikola", "admin"],
    bodyId: "labBody",
  },
};
const craftState = {
...
function renderCraftTab() {
  ...
  console.log(
    "[CREAR] currentPersonaje crudo:",
    JSON.stringify(currentPersonaje),
    "→ normalizado:",
    JSON.stringify(cp),
  );
  Object.entries(FACILITIES).forEach(([facId, fac]) => {
    ...
    if (!allowed) {
      body.innerHTML = `<div class="move-locked">Solo ${fac.allowed
        .filter((id) => id !== "admin")
        .map((id) => personajeLabel(id))
        .join(
          " o ",
        )} (y el DJ) ${fac.allowed.length > 2 ? "pueden" : "puede"} crear aquí. Cualquiera puede ver esta sección.</div>`;
      return;
    }
...
function renderMaterialRows(facId) {
  ...
  box.querySelectorAll(".craft-material-row").forEach((rowEl) => {
    const idx = parseInt(rowEl.dataset.idx, 10);
    rowEl
      .querySelector('[data-role="mat-select"]')
      .addEventListener("change", (ev) => {
        craftState[facId].materials[idx].invKey = ev.target.value;
      });
    rowEl
      .querySelector('[data-role="mat-amount"]')
      .addEventListener("change", (ev) => {
        craftState[facId].materials[idx].amount = Math.max(
          1,
          Math.floor(Number(ev.target.value)) || 1,
        );
      });
    rowEl
      .querySelector('[data-role="mat-remove"]')
      .addEventListener("click", () => {
...
function wireCraftForm(facId) {
  ...
  document
    .getElementById(`craft-${facId}-addStat`)
    .addEventListener("click", () => {
  ...
  document
    .getElementById(`craft-${facId}-addMat`)
    .addEventListener("click", () => addMaterialRow(facId));
  document
    .getElementById(`craft-${facId}-submit`)
    .addEventListener("click", () => submitCraft(facId));
}
...
async function submitCraft(facId) {
  ...
  const subcatId = subcatField.classList.contains("hidden")
    ? null
    : document.getElementById(`craft-${facId}-subcat`).value;
  const desc = document.getElementById(`craft-${facId}-desc`).value.trim();
  const qty = Math.max(
    1,
    Math.floor(Number(document.getElementById(`craft-${facId}-qty`).value)) ||
      1,
  );
  ...
  try {
    ...
    for (const m of materials) {
      ...
      const qtyRef = ref(
        db,
        `tienda/personajes/nave/inventario/${m.invKey}/qty`,
      );
      const result = await runTransaction(qtyRef, (current) => {
        const cur = typeof current === "number" ? current : 0;
      ...
      if (!result.committed) {
        throw new Error(
          `No hay suficiente "${invSnapBefore.name}" para consumir.`,
        );
      }
      if (result.snapshot.val() === 0) {
        await update(ref(db), {
          [`tienda/personajes/nave/inventario/${m.invKey}`]: null,
        });
      }
    ...
    if (existingKey) {
      await update(ref(db), {
        [`tienda/personajes/nave/inventario/${existingKey}/qty`]:
          (data[existingKey].qty || 0) + qty,
    ...
    await update(ref(db), {
      [`tienda/objetosLog/${logKey}`]: {
        ...
        personaje: currentPersonaje,
        facility: fac.label,
        materiales: materialNames.join(", ") || null,
        fecha,
        ts: Date.now(),
      },
    });
    ...
    document
      .getElementById(`craft-${facId}-materialsBox`)
...
/* =========================================================
   ...
   ========================================================= */
const invSectionState = {
  personal: { cat: null, sub: null },
  nave: { cat: null, sub: null },
};
...
function renderInventorySection(
  ...
  if (!activeHasItems) {
    const firstWithItems = cats.find(
      (c) => groups[c.id] && groups[c.id].length,
    );
  ...
  const subcatsDef =
    active === CATEGORIA_SIN_ID ? [] : SUBCATS_BY_CAT[active] || [];
  ...
  if (subcatsDef.length > 1) {
    ...
    const activeSubHasItems =
      activeSub && subGroups[activeSub] && subGroups[activeSub].length;
    if (!activeSubHasItems) {
      const firstSub = subList.find(
        (s) => subGroups[s.id] && subGroups[s.id].length,
      );
    ...
    subNavHtml =
      `<div class="subcat-nav inv-subcat-nav">` +
      subList
        .map((s) => {
  ...
  contentEl
    .querySelectorAll(".inv-subcat-nav button:not(.empty)")
    .forEach((b) => {
      b.addEventListener("click", () => {
...
function renderInventarioTab() {
  ...
  if (!currentUser || !currentPersonaje) {
    ...
    const personalInv =
      (personajesData[currentPersonaje] &&
        personajesData[currentPersonaje].inventario) ||
      {};
...
function init() {
  ...
  document
    .getElementById("moveDepositBtn")
    .addEventListener("click", () => moveCredits("deposito"));
  document
    .getElementById("moveWithdrawBtn")
    .addEventListener("click", () => moveCredits("retirada"));
  document
    .getElementById("moveTransferBtn")
    .addEventListener("click", transferCredits);

  document
    .getElementById("comprasSearch")
    .addEventListener("input", renderCompras);
  document
    .getElementById("comprasFilterPago")
    .addEventListener("change", renderCompras);
  document
    .getElementById("objetosSearch")
    .addEventListener("input", renderObjetosLog);
  document
    .getElementById("objetosFilterTipo")
    .addEventListener("change", renderObjetosLog);
